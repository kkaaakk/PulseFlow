...
import com.pulseflow.campaign.content.ContentResult;
import com.pulseflow.campaign.content.ContentVariant;
...
/**
 * Validates proposed content variants against authoritative promotion facts.
 *
...
@Slf4j
@Component
public class CampaignContentValidator {
     ...
     *
     * @param input the authoritative campaign facts (contains
     *              promotionFacts, titleMaxLength, bodyMaxLength,
     *              forbiddenWords, etc.)
     * @param result proposed content
     * @return cleaned result with invalid variants dropped
     * @throws IllegalArgumentException if all variants are invalid or critical
    ...
    public ContentResult validate(java.util.Map<String, Object> input, ContentResult result) {
        if (result == null || result.getVariants() == null || result.getVariants().isEmpty()) {
            throw new IllegalArgumentException("Content result has no variants");
        }
        ...
        if (kept.isEmpty()) {
            throw new IllegalArgumentException("All content variants failed fact validation");
        }
