...
This Python 3.11 service now has one Pydantic AI Growth Investigation Agent connected to Java's six [read-only internal tools](../docs/agent/agent-tool-contract.md). It creates per-run Evidence and a typed Diagnosis. See the [investigation contract](../docs/agent/investigation-domain.md). Java remains the authority for all business data and writes.
...
Set `PULSEFLOW_JAVA_BASE_URL` and `PULSEFLOW_AGENT_INTERNAL_TOKEN` even in offline mode. `GET /health/live` reports the process; `GET /health/ready` reports validated configuration and initialized runtime. Startup fails when required settings are missing. `POST /internal/v1/investigations` accepts `{ "question": "..." }` with the same internal Token header and returns Diagnosis, Evidence and Tool trajectory. It is intended for machine-to-machine calls, not browsers.
